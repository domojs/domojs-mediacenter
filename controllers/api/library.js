...
module.exports = {
    ...
    markAsRead: function( db, id, callback ) {
        ...
        else if( id.endsWith( ':episodes' ) )
            db.smembers( id, function( err, media ) {
                if( err )
                    callback( err );
                else {
                    ...
                    for( var i in media ) {
                        cmd.
                            hset( media[i], 'lastread', new Date().toISOString() ).
                            hincrby( media[i], 'readcount', 1 );
                    }
                    cmd.
                        hset( id, 'lastread', new Date().toISOString() ).
                        hset( id, 'lastmedia', id ).
                        exec( function( err ) {
                            debug( 'marked as read' );
                            callback( err )
                        });
    ...
    getArt: function( db, id, callback ) {
        var setArtwork = function( id, url, tmdb ) {
            $.ajax( { url: url }).on( 'response', function( res ) {
                ...
                res.on( 'end', function( chunk ) {
                    ...
                    db.hgetall(id, function(err, media){
                        var setter={};
                        if(!media.overview)
                            setter.overview=tmdb.overview;
                        setter.cover=img;
                        setter.tmdb=tmdb.id;
                        db.hset( id, setter, function( err ) {
                            if( err )
                                console.log( err );
                            else {
        ...
        db.hgetall( id, function( err, media ) {
            ...
            if( media.cover && media.cover != 'undefined' ) {
                if( media.cover.substr( 0, 4 ) == 'http' )
                    setArtwork( id, media.cover );
                else
                    callback( media.cover );
                return;
            }
            if( media.index ) {
            ...
            if( mediaType == 'music' ) {
                $.ajax( {
                    type: 'get', dataType: 'json', url: 'http://mb.videolan.org/ws/2/release/?fmt=json&query=artist:%22' + media.artist + '%22%20AND%20release:%22' + media.album + '%22', success: function( data ) {
                        ...
                        if( data.count > 0 ) {
                            ...
                            if( matchingAlbums.length > 0 ) {
                                return ( function( i ) {
                                    ...
                                    console.log( i );
                                    $.ajax( {
                                        type: 'get', dataType: 'json', url: 'http://mb.videolan.org/ws/2/release/' + matchingAlbums[ i ].id + '?fmt=json&inc=recordings', success: function( data ) {
                                            ...
                                            if( data.asin )
                                                ...
                                                $.ajax( {
                                                    type: 'get', dataType: 'json', url: 'http://coverartarchive.org/release/' + matchingAlbums[ i ].id, success: function( data ) {
                        ...
                        $.getJSON( 'http://private-f864a-themoviedb.apiary-proxy.com/3/search/tv?api_key=be3bc153ce74463263960789c93e29a9&query=' + media.name, function( results ) {
                            if( results.total_results > 0 ) {
                                $.eachAsync( results.results, function( i, item, next ) {
                                    if( media.season ) {
                                        $.ajax( 'http://private-f864a-themoviedb.apiary-proxy.com/3/tv/' + item.id + '/season/' + media.season + '?api_key=be3bc153ce74463263960789c93e29a9', {
                                            success: function( season ) {
                                                if( season.poster_path === null && media.season > 1 ) {
                                                    $.ajax( 'http://private-f864a-themoviedb.apiary-proxy.com/3/tv/' + item.id + '/season/1?api_key=be3bc153ce74463263960789c93e29a9', {
                                                        success: function( season ) {
                                                            setArtwork( id, config.images.base_url + 'original' + season.poster_path, item )
                                                        }, error: function( error ) {
                                                            console.log( error );
                                                            next();
                                                        }
                                        ...
                                        setArtwork( id, config.images.base_url + 'original' + item.poster_path, item );
                                    }
                                }, function() {
                                    callback( 404 );
                                });
