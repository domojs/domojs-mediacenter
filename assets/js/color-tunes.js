  ...
  var ColorTunes = {
	...
	redraw : function(image, canvas)
	{
		...
		/*$(canvas).css('max-width', image.clientWidth);
		$(canvas).css('max-height', image.clientHeight);*/
		...
		if($(canvas).hasClass('artwork-fadeout'))
			ColorTunes.fadeout(canvas, canvas.width, canvas.image, 0.5, bgColor);
		if($(canvas).hasClass('artwork-mirror'))
			ColorTunes.mirror(canvas, image.height - 1, canvas.height, bgColor);
		if($(canvas).hasClass('artwork-feathering'))
			ColorTunes.feathering(canvas, canvas.width, canvas.height, canvas.width*0.1, bgColor);
		return bgColor;
	},
	
	rgbaToCssString : function(alpha, color) {
    ...
    launch : function(image, canvas, callback) {
      var bgColor, color, dist, fgColor, fgColor2, fgColorMap, fgPalette, maxDist, _i, _j, _len, _len1;
        ...
        fgColorMap = ColorTunes.getColorMap(canvas, 0, 0, canvas.width, canvas.height, 10);
		...
		callback(bgColor, fgColor, fgColor2);
    }
	};