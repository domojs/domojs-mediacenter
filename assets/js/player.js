...
function switchPlayer( newPlayer ) {
    ...
    player = newPlayer.id;
    $( '.player-switch .btn-label' ).text( player.name );
...
socket.on( 'iamaplayer', function( identity ) {
    if( $( '.player-switch' ).length - $( '.player-switch li.player-' + identity.id ).length == 0 )
        return;
    $( '.player-switch:not(:has(li.player-' + identity.id + ')) ul.dropdown-menu' ).append( $('<li class="player-' + identity.id + '"><a>' + identity.name + '</a></li>' ).click( function() {
